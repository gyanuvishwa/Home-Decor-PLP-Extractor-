# WORKER BRIEF — Home Decor Category & Qty Extraction (Final_Company_List, S.N. 91–100)
...
|---|---|---|---|
| 91 | Next Home | next.co.uk/homeware | UK |
| 92 | Marks & Spencer Home | marksandspencer.com | UK |
| 93 | Barker and Stonehouse | barkerandstonehouse.co.uk | UK |
| 94 | Loaf | loaf.com | UK |
| 95 | Furniture Village | furniturevillage.co.uk | UK |
| 96 | Oliver Bonas Home | oliverbonas.com | UK |
| 97 | H&M Home UK | www2.hm.com/en_gb/home.html | UK |
| 98 | Zara Home UK | zarahome.com/gb/ | UK |
| 99 | Westwing UK | westwing.co.uk | UK |
| 100 | IKEA UK | ikea.com/gb/en/ | UK |

**Company-specific notes:**
- **S.N. 98 Zara Home UK is a DIFFERENT roster entry from S.N. 21 Zara Home UAE**, already
  extracted (`https://www.zarahome.com/ae/`, UAE, 35 leaf/5 parent rows, in
  `pipeline/final_list_batch_sn11-20/c21.json`). Zara Home runs one global Inditex platform
  with per-country storefronts (catalog/store IDs embedded in the URL, per the S.N. 21
  worker's notes) — **verify independently whether the UK storefront is a genuinely
  separate catalogue/currency/store-ID (most likely, given Inditex runs real per-country
  inventory and GBP pricing) or somehow collapses into the same catalogue as UAE.** Don't
  assume either way; check store/catalog IDs and currency the way the S.N. 82 OKA US and
  S.N. 21 workers did. If Akamai/proof-of-work blocking recurs (Zara Home UAE needed an
  inline-JS interstitial solver, preserved at
  `pipeline/final_list_batch_sn21-30/inditex_interstitial_solver.py`), that solver may be
  directly reusable since it's the same Inditex platform family.
  - **S.N. 95 Furniture Village and S.N. 94 Loaf** are furniture-led DTC/specialist
    retailers — check thoroughly for any lighting/mirror/decor-object line before
    concluding a low or zero-row result; a genuine low count is still legitimate if that's
    really all they sell (same precedent as Burrow/Joybird in the S.N. 71-80 batch).
  - **S.N. 92 Marks & Spencer Home, S.N. 100 IKEA UK** are large multi-category retailers'
    home sections — expect large numbers, verify carefully against inflation using the
    empirical exhaustion checks in Rule 1. IKEA in particular is known for enormous
    catalogues; do not let volume alone justify skipping the exhaustion check on any single
    category.
  - **S.N. 93 Barker and Stonehouse, S.N. 96 Oliver Bonas Home, S.N. 99 Westwing UK** are
    boutique/curated home-decor-forward retailers — likely real, substantial coverage
    across lighting/mirrors/decorative accessories.
  - **S.N. 97 H&M Home UK** is a fast-fashion retailer's home line — real decor coverage is
    plausible (H&M Home is a recognized standalone product line) but verify scope; watch
    especially for the project's standing Home Fragrance / artificial-plant exclusions,
    which fast-fashion home lines tend to lean into heavily.

No site-specific access hints are known yet for any of these ten except the Zara Home UAE
precedent above (same platform family, different country storefront). Budget time for an
access-route discovery sweep (Rule 2) on each.
