# WORKER BRIEF — Home Decor Category & Qty Extraction (Final_Company_List, S.N. 111–120)
...
|---|---|---|---|
| 111 | Fleux | fleux.com | France |
| 112 | Westwing DE | westwing.de | Germany |
| 113 | Home24 DE | home24.de | Germany |
| 114 | Hoeffner | hoeffner.de | Germany |
| 115 | XXXLutz DE | xxxlutz.de | Germany |
| 116 | Zara Home DE | zarahome.com/de/ | Germany |
| 117 | IKEA DE | ikea.com/de/de/ | Germany |
| 118 | Connox | connox.com | Germany |
| 119 | KARE | kare.de | Germany |
| 120 | Butlers | butlers.com | Germany |

**Company-specific notes:**
- **This batch moves into Germany (S.N. 112-120) plus one more French site (S.N. 111
  Fleux).** See the LANGUAGE / NAMING section above — translate French/German category
  names into plain English; do not leave French/German text or accented characters/umlauts
  in `category`/`sub_category`. Expect EUR pricing throughout; that's expected and not
  itself evidence of a different catalogue from any related site.
- **Three companies this batch are likely related to already-extracted companies — verify
  independence for each, following the same discipline as the S.N. 82 OKA US, S.N. 98 Zara
  Home UK, and S.N. 106 La Redoute Interieurs FR precedents (read the earlier company's
  worker JSON first, then check store/catalog IDs, currency, and product-ID overlap before
  deciding — don't assume either way):**
  - **S.N. 116 Zara Home DE** vs S.N. 21 Zara Home UAE
    (`pipeline/final_list_batch_sn11-20/c21.json`) and S.N. 98 Zara Home UK
    (`pipeline/final_list_batch_sn91-100/c98.json`, confirmed genuinely separate from UAE
    via distinct store/catalog IDs). Same Inditex platform family — the interstitial solver
    at `pipeline/final_list_batch_sn21-30/inditex_interstitial_solver.py` may be directly
    reusable if the same Akamai proof-of-work challenge recurs.
  - **S.N. 112 Westwing DE** vs S.N. 99 Westwing UK
    (`pipeline/final_list_batch_sn91-100/c99.json`, 68 leaf/11 parent, custom Shopify
    Oxygen/Hydrogen storefront, category tree from `/sitemap-categories.xml`).
  - **S.N. 117 IKEA DE** vs S.N. 100 IKEA UK
    (`pipeline/final_list_batch_sn91-100/c100.json`, 58 leaf/10 parent, every qty verified
    via IKEA's own search API with unique-ID exhaustion + boundary tests). IKEA runs one
    global product platform with per-country catalogs/pricing — the API pattern discovered
    for the UK site (`sik.search.blue.cdtapps.com`, `PRIMARY_AREA` component) is very likely
    reusable directly for the DE site, just check the country/market parameter.
- **After the S.N. 109 Alinea finding last batch (a French company roster row that turned
  out to be a liquidation-scam domain, not the real retailer), do a quick Rule 3 sanity
  check on every company in this batch too** — verify the live site is genuinely who it
  claims to be (real company registration/legal notices, no "closing down sale" banner
  language, matches independently known facts about the brand) before extracting anything.
  This applies to all ten, not just ones that seem suspicious upfront.
- **S.N. 113 Home24 DE, S.N. 114 Hoeffner, S.N. 115 XXXLutz DE** are large German
  multi-category furniture/home retailers — expect large numbers, verify carefully against
  inflation using the empirical exhaustion checks in Rule 1 rather than assuming any
  structured count API total is exact. None of these platforms have been touched by this
  project before.
- **S.N. 111 Fleux, S.N. 118 Connox, S.N. 119 KARE, S.N. 120 Butlers** are
  boutique/curated/designer home-decor-forward retailers — likely real, substantial
  coverage across lighting/mirrors/decorative accessories. KARE in particular is a
  well-known German design-object/furniture brand with a large decorative-accessories line.

No site-specific access hints are known yet for any of these ten except the Zara
Home/Westwing/IKEA precedents above (related companies, different country storefronts).
Budget time for an access-route discovery sweep (Rule 2) on each — German retail sites may
use different WAF vendors than the UK/US/France sites seen so far in this project, so don't
assume the same profile sweep order will work first try.
...
- Keep `sub_category` faithful to the site's own naming **translated into plain English** —
  this batch includes one French site (S.N. 111 Fleux) and nine German sites (S.N. 112-120).
  Translate the site's own French/German category name into natural English (e.g.
  `Luminaires`/`Beleuchtung` -> `Lighting`, `Miroirs`/`Spiegel` -> `Mirrors`,
  `Vases et Contenants`/`Vasen` -> `Vases`); do not leave French/German text or
  transliterate accents/umlauts (e.g. `Decoration` not `Décoration`, `Hoeffner`/`Hoffner`
  not `Höffner` if a company name itself needs an ASCII form anywhere you write it — though
  note `company`/`brand_site` fields are NOT subject to this ASCII rule, only `category`/
  `sub_category` are; this proved out cleanly across all 10 French-site workers in the
  S.N. 101-110 batch, same approach applies here). `evidence` and `notes` may quote the
  original French/German text/UI strings for verification purposes, but `category`/
  `sub_category` themselves must be ASCII-only English strings, same as every other
