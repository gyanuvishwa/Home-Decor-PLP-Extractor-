...
disp = set(int(x) for x in open(disp_path)) if os.path.exists(disp_path) else set()
done = set()
for f in glob.glob(os.path.join(SP, 'ts*.json')):
    m = re.match(r'^ts(\d+)\.json$', os.path.basename(f))
    if m:
