# WORKER BRIEF — Home Decor Category & Qty Extraction (Final_Company_List, S.N. 81–90)
...
|---|---|---|---|
| 81 | Safavieh Home | safaviehhome.com | USA |
| 82 | OKA US | oka.com (US/root locale) | USA |
| 83 | Bloomingdale's Home | bloomingdales.com/shop/home?id=3865 | USA |
| 84 | Container Store Decor | containerstore.com/home-decor | USA |
| 85 | Overstock Home | overstock.com/Home-Garden/Home-Decor/248/cat.html | USA |
| 86 | QVC Home | qvc.com/home-and-garden/_/N-17h8/c.html | USA |
| 87 | HSN Home | hsn.com/shop/home/ho0354 | USA |
| 88 | Cost Plus World Market | worldmarket.com | USA |
| 89 | Graham and Green | grahamandgreen.co.uk | UK |
| 90 | La Redoute UK | laredoute.co.uk | UK |

**Company-specific notes:**
- **S.N. 82 OKA US is a DIFFERENT roster entry from S.N. 53 OKA**, already extracted
  (`https://www.oka.com/uk`, UK, 21 leaf/4 parent rows, in
  `pipeline/final_list_batch_sn51-60/c53.json`). S.N. 82's domain is the bare
  `https://www.oka.com/` root, presumably the US/default locale. **Verify independently
  whether this is a genuinely separate US catalogue/currency/SKU set (like S.N. 61 Pottery
  Barn Kids UK turned out to be a distinct backend from its US sibling) or the same global
  catalogue merely reached via a different URL (like S.N. 28 HAY US, which redirected into
  an existing catalogue).** Do not assume either way — check locale switching, currency,
  and whether product IDs overlap with S.N. 53's known set before deciding whether to
  extract fresh rows or note it as effectively the same catalogue.
- **S.N. 88 Cost Plus World Market** may be the same company as **S.N. 27 World Market**
  (already extracted: `worldmarket.com`, 41 leaf/8 parent, Salesforce Commerce Cloud, in
  `pipeline/final_list_batch_sn21-30/c27.json`) — "Cost Plus World Market" was the brand's
  former legal name. **Check the domain the roster gives you (worldmarket.com) against
  S.N. 27's — if they resolve to the identical live site, this is very likely a duplicate
  roster row for the same company, not a fresh target.** If confirmed identical, write
  `c88.json` with `"status": "ok"`, `"rows": []`, and a note explaining it's the same
  catalogue as S.N. 27 with evidence (e.g. identical product counts, identical nav) —
  don't re-extract and don't silently skip without documenting why.
- **S.N. 83 Bloomingdale's Home, S.N. 84 Container Store Decor, S.N. 85 Overstock Home,
  S.N. 86 QVC Home, S.N. 87 HSN Home** are department-store/mass-marketplace home sections
  inside much larger multi-category retailers — expect large numbers, verify carefully
  against inflation using the empirical exhaustion checks in Rule 1 rather than assuming
  any structured count API total is exact. None of these platforms have been touched by
  this project before — start fresh, no known access pattern to reuse. Note Overstock is
  now rebranded "Bed Bath & Beyond" for some product lines per the S.N. 47 Bed Bath & Beyond
  worker's finding (`bedbathandbeyond.com` now runs Beyond Inc.'s ex-Overstock platform) —
  check whether overstock.com itself is still live and distinct, or redirects into the same
  Beyond Inc. platform already captured at S.N. 47; if it's the same underlying catalogue,
  document that rather than double-counting.
- **S.N. 81 Safavieh Home** is primarily a rugs/furniture brand — check thoroughly for any
  lighting/mirror/decor-object line before concluding a low or zero-row result; a genuine
  low count is still legitimate if that's really all they sell.
- **S.N. 89 Graham and Green, S.N. 90 La Redoute UK** are both UK sites — Graham and Green
  is a boutique home-decor specialist (likely real coverage across lighting/mirrors/decor);
  La Redoute is a large French-owned multi-category retailer's UK storefront (mostly
  fashion/furniture — check for a genuine, possibly modest, home-decor department before
  concluding low/zero).

No site-specific access hints are known yet for any of these ten — none have been touched
by this project before. Budget time for an access-route discovery sweep (Rule 2) on each.
