#!/usr/bin/env python3
"""Build Pet_Care.xlsx from the pet-care worker JSON (pc<SR>.json).
...
Writes a BRAND NEW workbook. Never opens, reads or touches:
  ...
  Storage.xlsx / Final_Company_List (1).xlsx  (read-only for scope)
...
Sheets written:
  ...
  Review            - leaf rows with null qty, a MANUAL REVIEW flag, or an
                       unresolved consumable/medical classification

Trusts the worker's Pet Care classification (rules/pet-care.md §1-10, §24-25) -
this merge does not re-derive category membership the way merge_furniture.py's
NOT_FURNITURE net does, because Pet Care is an inclusion-based taxonomy, not an
exclusion-based one, and no executable classifier equivalent to classify.py
exists for it. It only nets: navigation/marketing nodes (rules §13), the two
open policy gaps (consumables, medical/veterinary - rules §11/§12), and the
same structural hygiene (dedup key, qty+evidence, link validity, ASCII-only)
every category shares.

Idempotent: rebuilds everything from whatever pc*.json is on disk.

Usage:  python merge_pet_care.py
"""
...
# (rules/pet-care.md §13, shared with the core's nav/marketing exclusions).
BANNED = re.compile(
...
# Rules §11: no include/exclude policy for pet-care consumables yet. Force the
# review flag rather than trust the worker remembered it.
CONSUMABLE_FLAG = re.compile(
    r'\b(pet\s?)?(food|treats?|snacks?|chews?|supplements?|vitamins?|nutrition)\b', re.I)
# Feeding EQUIPMENT (bowls, storage, canisters, mats, jars, feeders) is a Tier 1
# strict include per rules/pet-care.md ("Pet Bowls", "Pet Food Storage", "Pet
# Treat Storage") -- it is not the consumable itself. "Food Bowls" / "Dog Food
# Storage" / "Pet Food Canisters" must NOT be force-flagged just because they
# contain the word "food"/"treat".
FEEDING_EQUIPMENT_OVERRIDE = re.compile(
    r'\b(bowls?|storage|canisters?|mats?|jars?|containers?|dish(es)?|feeders?|scoops?)\b', re.I)

# Rules §14 (Toys & enrichment) lists "Chew Toys" and "Interactive/Puzzle/Treat
# Toys" as ordinary Tier 1 includes -- the toy IS the product, "chew"/"treat" is
# just an adjective describing it, not a separate consumable being sold. Strip
# any "<adjective> toy(s)" phrase out of the name before testing for a
# consumable, so "Chew Toys" is cleared but "Toys & Treats" / "Pet Toys &
# Treats" (treats listed as a SEPARATE item alongside toys, not describing them)
# still leaves a bare "treats" behind and stays flagged. Added 2026-09-02 after
# 'Chew Toys' was wrongly force-flagged in SR 131 (OTTO Home) -- same failure
# mode as the FEEDING_EQUIPMENT_OVERRIDE bug, see qa_notes.md.
...
# Rules §12: no inclusion rule for medical/veterinary categories yet.
...
MANUAL_REVIEW_CONSUMABLE = "MANUAL REVIEW: PET CONSUMABLE - no include/exclude policy yet"
MANUAL_REVIEW_MEDICAL = "MANUAL REVIEW: PET MEDICAL / VETERINARY CATEGORY - no inclusion rule yet"

# The master roster lists these SR numbers twice for the identical company
# (same name/brand_site/URL) -- confirmed for SR 286 = SR 165 "Noon UAE"
# (same duplicate already resolved identically in the sibling Furniture
# project, furniture_json_archive/qa_notes.md, user decision 2026-08-24).
# The duplicate SR is dropped entirely (not even a ledger row) per the
# user's explicit instruction 2026-09-02 ("sr 286 and 165 are same
# companies so just removed one of them") -- the canonical SR keeps the
# real data. Ledger total is intentionally 286 - len(this dict).
...
def load_blocks(scope):
    ...
    for c in scope:
        sr = c["sr"]
        p = os.path.join(SCRATCH, f"pc{sr}.json")
        if not os.path.exists(p):
            blocks.append({**c, "status": "MISSING", "failure_reason":
                           f"no pc{sr}.json produced", "notes": None, "rows": []})
            problems.append(f"SR {sr}: NO FILE (pc{sr}.json missing)")
            continue
        try:
        ...
        for r in d.get("rows") or []:
            ...
            cat = (r.get("category") or "").strip() or "Pet Care"
            ...
            if not is_parent:
                if is_consumable(sub) and "CONSUMABLE" not in (flag or ""):
                    problems.append(f"SR {sr}: '{sub}' looks like a consumable, no policy -> flagged")
                    flag = ((flag + " | ") if flag else "") + MANUAL_REVIEW_CONSUMABLE
                if MEDICAL_FLAG.search(sub) and "MEDICAL" not in (flag or "") and "VETERINARY" not in (flag or ""):
                    problems.append(f"SR {sr}: '{sub}' looks medical/veterinary, no rule -> flagged")
                    flag = ((flag + " | ") if flag else "") + MANUAL_REVIEW_MEDICAL
            ...
            if is_parent:
                ...
                # different listing page - keep both (shared merge rule, same trap
                # merge_furniture.py's dedup key exists to avoid).
...
def ledger_status(b):
    """Exactly one of the four shared outcomes."""
    ...
    # ok / partial
    return "PET CARE FOUND" if b["rows"] else "NO PET CARE FOUND"


def main():
    for f in ("Pet_Care.xlsx",):
    ...
    LHDR = ["Maisons", "Company", "Country", "Website", "Pet Care Found",
            "Pet Care Record Count", "Status", "Notes"]
    lw.append(LHDR)
    for c in range(1, len(LHDR) + 1):
        ...
        cell.border = CELL_BORDER
        cell.alignment = Alignment(vertical="center", horizontal="center")
    counts = {"PET CARE FOUND": 0, "NO PET CARE FOUND": 0,
