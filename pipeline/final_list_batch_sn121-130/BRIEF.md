# WORKER BRIEF — Home Decor Category & Qty Extraction (Final_Company_List, S.N. 121–130)
...
|---|---|---|---|
| 121 | fonQ | fonq.nl | Netherlands |
| 122 | vtwonen | vtwonen.nl/shop | Netherlands |
| 123 | HKliving | hk-living.com | Netherlands |
| 124 | Made.com NL | made.com/nl/ | Netherlands |
| 125 | Juttu Home | juttu.be/nl/home | Belgium |
| 126 | Maisons du Monde BE | maisonsdumonde.com/BE/nl | Belgium |
| 127 | Zara Home ES | zarahome.com/es/ | Spain |
| 128 | Kave Home | kavehome.com/es/en/ | Spain |
| 129 | El Corte Ingles Home | elcorteingles.es/hogar/ | Spain |
| 130 | Sklum | sklum.com/es/ | Spain |

**Company-specific notes:**
- **This batch moves into the Netherlands (S.N. 121-124), Belgium (S.N. 125-126), and Spain
  (S.N. 127-130).** See the LANGUAGE / NAMING section above — translate Dutch/Spanish
  category names into plain English; do not leave Dutch/Spanish text or accented characters
  in `category`/`sub_category`. Expect EUR pricing throughout; that's expected and not
  itself evidence of a different catalogue from any related site. Juttu Home's assigned URL
  path (`/nl/home`) and Maisons du Monde BE's (`/BE/nl`) both suggest Dutch-language Belgian
  storefronts specifically (Belgium is multilingual FR/NL) — confirm which language the
  assigned locale actually renders before translating, don't assume.
- **Three companies this batch are likely related to already-extracted companies — verify
  independence for each, following the same discipline as the S.N. 82 OKA US, S.N. 98/116
  Zara Home UK/DE, and S.N. 106/112 La Redoute/Westwing precedents (read the earlier
  company's worker JSON first, then check store/catalog IDs, currency, and product-ID
  overlap before deciding — don't assume either way):**
  - **S.N. 127 Zara Home ES** vs the three already-confirmed-distinct Zara Home country
    catalogues: S.N. 21 UAE (`pipeline/final_list_batch_sn11-20/c21.json`), S.N. 98 UK
    (`pipeline/final_list_batch_sn91-100/c98.json`), S.N. 116 DE
    (`pipeline/final_list_batch_sn111-120/c116.json`, store 84009904/catalog 80290000).
    Spain is Zara/Inditex's home market, so ES may have the largest catalogue of the four —
    verify its own store/catalog ID rather than assuming. The interstitial solver at
    `pipeline/final_list_batch_sn21-30/inditex_interstitial_solver.py` has now worked
    unmodified on every Zara Home country site attempted; try it first if the Akamai
    proof-of-work challenge recurs.
  - **S.N. 124 Made.com NL** vs S.N. 51 Made.com UK
    (`pipeline/final_list_batch_sn51-60/c51.json`) — the UK site was found to be live but
    no longer Made.com's original catalogue; it's now a Next Retail storefront behind the
    same domain (a change of owner, not a redirect). Check independently whether made.com/nl/
    is the same Next Retail situation, a genuinely separate Dutch catalogue, or something
    else entirely (e.g. retired) — read `c51.json` first for the UK finding, but don't
    assume NL mirrors it.
  - **S.N. 126 Maisons du Monde BE** vs S.N. 104 Maisons du Monde FR
    (`pipeline/final_list_batch_sn101-110/c104.json`, `Partial` — DataDome hard-blocked
    every count route there after an exhaustive Rule 2 sweep). Read `c104.json` first: the
    category STRUCTURE that was recovered there (36 leaf + 7 parent nodes, Rule 0 gains on
    outdoor lighting) is very likely to carry over to BE on the same platform, which may
    save rediscovery time even if BE also gets blocked. Independently verify whether BE is
    reachable where FR was not — worth a fresh, full Rule 2 sweep regardless since access
    can differ by country/CDN edge even on the same underlying platform.
- **Continue the Rule 3 sanity-check habit from recent batches on every company here too**
  (verify the live site is genuinely who it claims to be — real registration/legal notices,
  no closing-down-sale language) — this caught a scam domain at S.N. 109 Alinea previously.
- **Continue checking `robots.txt` for a Claude-specific disallow** (`ClaudeBot`,
  `anthropic-ai`, `Claude-Web`) before any WAF-bypass attempt on every company — this
  caught real cases at Made In Design and Home24 DE in earlier batches; if found, mark
  `failed` with the finding as the reason rather than bypassing it.
- **S.N. 129 El Corte Ingles Home** is a huge Spanish department-store chain's home
  section — expect large numbers, verify carefully against inflation using the empirical
  exhaustion checks in Rule 1 rather than assuming any structured count API total is exact.
- **S.N. 121 fonQ, S.N. 128 Kave Home, S.N. 130 Sklum** are large home-decor/furniture
  e-commerce platforms (fonQ is a major Dutch home/lifestyle marketplace; Kave Home and
  Sklum are Spanish design-furniture DTC brands with substantial decor lines) — expect
  real, possibly large, decor coverage.
- **S.N. 122 vtwonen, S.N. 123 HKliving, S.N. 125 Juttu Home** are
  boutique/curated/designer home-decor-forward retailers (vtwonen is a well-known Dutch
  interiors media brand with a retail spinoff) — likely real, substantial coverage across
  lighting/mirrors/decorative accessories.

No site-specific access hints are known yet for any of these ten except the Zara
Home/Made.com/Maisons du Monde precedents above (related companies, different country
storefronts). Budget time for an access-route discovery sweep (Rule 2) on each — Dutch,
Belgian and Spanish retail sites may use different WAF vendors than sites seen so far in
this project, so don't assume the same profile sweep order will work first try.
...
- Keep `sub_category` faithful to the site's own naming **translated into plain English** —
  this batch spans four countries/languages: Netherlands/Dutch (S.N. 121-124), Belgium
  (S.N. 125-126, likely Dutch-language storefronts based on the `/nl/` URL paths), and
  Spain/Spanish (S.N. 127-130). Translate the site's own Dutch/Spanish category name into
  natural English (e.g. `Verlichting` (Dutch) / `Iluminacion` (Spanish) -> `Lighting`,
  `Spiegels`/`Espejos` -> `Mirrors`, `Vazen`/`Jarrones` -> `Vases`); do not leave
  Dutch/Spanish text or transliterate accents (e.g. `Decoracion` not `Decoración`). This
  translate-to-English approach has now proven out cleanly across French (S.N. 101-110)
  and German (S.N. 111-120) batches — same method applies here. `evidence` and `notes` may
  quote the original Dutch/Spanish text/UI strings for verification purposes, but
  `category`/`sub_category` themselves must be ASCII-only English strings, same as every
  other company in this project.
