# WORKER BRIEF — Home Decor Category & Qty Extraction (Final_Company_List, S.N. 11–20)
...
## WORKSPACE RULE — READ FIRST (new this batch)

**Create and work inside your OWN subdirectory: `scratchpad\w<SR>\`** (e.g. `w11\`).
Put every scratch file, script, HTML dump and cache in there.

Last batch, workers used generic filenames in the shared root and **clobbered each
other mid-run** — `build.py`, `retry.py`, `cio.py` and `counts.json` were all
overwritten by other workers, causing parse failures and lost state. Your subdirectory
makes that impossible. The only file you write to the scratchpad **root** is your final
`c<SR>.json`.
...
**Confirmed across three platforms and ten companies. The convenient structured number
counts a different population than the one shoppers see, and it is always higher.**
...
- **Constructor.io `ac.cnstrc.com` → `total_num_results`** — inflated on all 4
  Williams-Sonoma properties tested (Pottery Barn: exact on 17 of 41 categories,
  inflated on the other 24).
- **Bloomreach `core.dxpapi.com`** — returns HTTP 451 from here; unusable.
...
**The trap that nearly beat us last batch, so you don't repeat it:** a worker read the
storefront JS bundle, proved the code maps the API's `total_num_results` straight to the
rendered header, and concluded the two must be identical. **They were not** — 6
divergences. The storefront's own request carries session/segment context that your
outside API call does not reproduce. *Reading the code proves what the page renders, not
what your request returns.* Only empirical comparison settles it. If you find yourself
reasoning from code to a count, stop and compare against the rendered page instead.
...
**Do NOT infer "unreachable" from a missing AAAA record.** That guidance was in last
batch's brief and it was wrong: `potterybarn.com` has **no AAAA and connects fine** —
its 403 is a genuine Akamai block. A missing AAAA is a hint about *which route to try*,
never evidence about *why a request failed*. Test reachability by actually fetching.

**Routes, in the order they succeeded last batch:**

1. **Direct fetch.** Try it first. It worked for Article, McGee & Co. and Interior
   Define, all of which earlier notes predicted would fail.
2. **`r.jina.ai/<url>` reader proxy** — the workhorse. Carried Anthropologie, RH, PB
   Kids, PB Teen, Pottery Barn and West Elm past Akamai/URBN 403s, and renders SPA
   storefronts *including their product-count headers*.
   ...
   **wait and retry**. One worker last batch gave up on it and fell back to six-month-old
   Wayback snapshots; a later retry succeeded and verified all 35 of its URLs live.
3. **`translate.goog`** —
...
4. **Cloudflare IPv6 anycast pinning + `curl_cffi`** — cracked Wayfair when everything
   else was dead. Pin `CURLOPT_RESOLVE` for the host to a Cloudflare edge
   (`2606:4700::6810:84e5`) with correct SNI, then fetch with
   `curl_cffi` `impersonate="chrome"` (already in the project venv). Plain
   curl/requests hit PerimeterX; `curl_cffi` passes cleanly.
...
**Mixed roll-ups: drop them, don't pad.** Last batch PB Kids dropped "Nursery Decor"
(240 items) because it bundled wallpaper, hampers and storage in with decor, and PB Teen
dropped "Clocks & Desk Accessories" (108) because its own H1 was "Teen Desk Accessories"
and it was mostly functional desk goods. Taking either would have inflated coverage with
...
- **Output must be plain English only.** No non-ASCII in `category` or `sub_category`.
  ...
  **Relevant this batch: UAE sites may serve Arabic — always use the `/en` locale**
  and English category names.
...
```json
{
  "sr": 11,
  "company": "Arhaus",
  "brand_site": "arhaus.com",
  "country": "USA",
  "site_url": "https://www.arhaus.com/",
  "status": "ok",
  "failure_reason": null,
  "notes": "Counts from rendered listing header 'N Results'. Reached directly.",
  "rows": [
    {"category":"Lighting","sub_category":"Ceiling Lighting","parent":true,
     "qty":null,"link":null,"evidence":null,"flag":null},
    {"category":"Lighting","sub_category":"Chandeliers","parent":false,
     "qty":124,"link":"https://www.arhaus.com/lighting/chandeliers/",
...
`python -c "import json;d=json.load(open('c11.json',encoding='utf-8'));print(len(d['rows']))"`
...
a short list, or an empty one with `"status": "ok"` and an explanatory note, is the
correct answer when the catalogue genuinely has little in-scope decor.
