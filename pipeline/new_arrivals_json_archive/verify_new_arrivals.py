#!/usr/bin/env python3
"""Post-merge validation of New_Arrivals.xlsx + protected-file check."""
...
# extracted at SR 165 ("Noon UAE") -- dropped entirely by merge_new_arrivals.py.
...
def main():
    ...
    # New-Collection ambiguity cross-check: every bare "New Collection"-style
    # sub-category without a clear New Arrivals signal must be on Review.
    # Excludes grouping rows -- merge_new_arrivals.py's forced flag only
    # applies to leaf rows (a grouping row never carries its own qty/link and
    # is never added to Review by construction).
    ambiguous = [(r[5], r[6]) for r in rows
                 if r[6] and needs_collection_review(str(r[6]))
                 and not (r[8] is None and not r[9])]
    print(f"  bare 'Collection'-style sub-categories not confirmed New Arrivals: {len(ambiguous)}  (must all appear on Review)")
    ...
    assert not banned_leaked, (
        ...
        "merge_new_arrivals.py's BANNED filter should have caught these"
    )
