# WORKER BRIEF — Home Decor Category & Qty Extraction (Final_Company_List, S.N. 101–110)
...
|---|---|---|---|
| 101 | Rockett St George | rockettstgeorge.co.uk | UK |
| 102 | House of Hackney | houseofhackney.com | UK |
| 103 | Amara | amara.com | UK |
| 104 | Maisons du Monde FR | maisonsdumonde.com/FR/fr | France |
| 105 | AM.PM France | ampm.fr | France |
| 106 | La Redoute Interieurs FR | laredoute.fr | France |
| 107 | Conforama FR | conforama.fr | France |
| 108 | Camif | camif.fr | France |
| 109 | Alinea | alinea-stores.com | France |
| 110 | The Socialite Family | thesocialitefamily.com/en-eu/ | France |

**Company-specific notes:**
- **This batch is the project's first move into non-UK/US/UAE sites (France, S.N.
  104-110).** See the LANGUAGE / NAMING section above — translate French category names
  into plain English; do not leave French text or accented characters in `category`/
  `sub_category`. Expect EUR pricing throughout; that's expected and not itself evidence of
  a different catalogue from any related site.
- **S.N. 106 La Redoute Interieurs FR may be related to S.N. 90 La Redoute UK**, already
  extracted (`https://www.laredoute.co.uk/`, UK, 23 leaf/1 parent rows, in
  `pipeline/final_list_batch_sn81-90/c90.json`). La Redoute is a French-origin retailer
  with country storefronts, similar in shape to the Zara Home / OKA precedents from earlier
  batches. **Verify independently whether laredoute.fr is a genuinely separate
  catalogue/currency/store-ID from the UK site (most likely, given a full French vs UK
  storefront split) or somehow the same catalogue.** Read `c90.json` first for context,
  then check store/catalog IDs, currency, and product-ID overlap before deciding — don't
  assume either way, same discipline as the S.N. 82 OKA US and S.N. 98 Zara Home UK cases.
  Note the roster's own domain differs materially (`.fr` vs `.co.uk`, and the assigned URL
  is a specific `/pplp/.../cat-158093.aspx` category path, not a homepage) — this on its
  own suggests a distinct catalogue, but confirm with real evidence.
- **S.N. 107 Conforama FR and S.N. 109 Alinea** are large French multi-category
  furniture/home retailers — expect large numbers, verify carefully against inflation using
  the empirical exhaustion checks in Rule 1 rather than assuming any structured count API
  total is exact. Neither platform has been touched by this project before.
- **S.N. 101 Rockett St George, S.N. 102 House of Hackney, S.N. 103 Amara, S.N. 105 AM.PM
  France, S.N. 110 The Socialite Family** are boutique/curated/designer home-decor-forward
  retailers — likely real, substantial coverage across lighting/mirrors/decorative
  accessories. House of Hackney in particular is known for maximalist wallpaper and print
  design — remember wallpaper itself is a standing exclusion even here.
- **S.N. 104 Maisons du Monde FR** is a large French home-decor specialist (sister markets
  exist in other EU countries, but this roster row is specifically the FR/.fr storefront) —
  expect substantial genuine decor coverage, verify counts carefully.
- **S.N. 108 Camif** is a French home-goods retailer with an ethical/local-manufacturing
  positioning — establish its actual current catalogue scope independently, no prior
  assumption either way.

No site-specific access hints are known yet for any of these ten except the La Redoute UK
precedent above (related company, different country storefront). Budget time for an
access-route discovery sweep (Rule 2) on each — French retail sites may have different WAF
vendors than the UK/US sites seen so far in this project, so don't assume the same profile
sweep order will work first try.
...
- Keep `sub_category` faithful to the site's own naming **translated into plain English** —
  this batch includes French sites (S.N. 104-110). Translate the site's own French category
  name into natural English (e.g. `Luminaires` -> `Lighting`, `Miroirs` -> `Mirrors`,
  `Vases et Contenants` -> `Vases and Containers`); do not leave French text or transliterate
  accents (e.g. `Decoration` not `Décoration`). `evidence` and `notes` may quote the
  original French text/UI strings for verification purposes, but `category`/`sub_category`
  themselves must be ASCII-only English strings, same as every other company in this
  project. Quoting the original French text inside `evidence` (e.g. "rendered header read
  '124 articles'") is fine — the ASCII/English rule applies only to `category` and
  `sub_category`.
