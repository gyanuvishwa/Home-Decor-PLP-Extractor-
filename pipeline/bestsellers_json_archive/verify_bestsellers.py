#!/usr/bin/env python3
"""Post-merge validation of Bestsellers.xlsx + protected-file check."""
...
# extracted at SR 165 ("Noon UAE") -- dropped entirely by merge_bestsellers.py,
# same fix as Furniture/Pet Care/Seasonal/Bathroom. Ledger is intentionally
...
def main():
    ...
    # PDP-shaped links that slipped through without a flag -- cross-check that
    # merge_bestsellers.py's PDP_LIKE safety net actually caught them.
    pdp = [(r[5], r[6], r[9]) for r in rows if r[9] and PDP_LIKE.search(str(r[9]))]
    print(f"  PDP-shaped links (must all be on Review): {len(pdp)}")
    for x in pdp[:20]:
        print("      ", x)

    # nav/marketing-shaped sub-categories that slipped into Output -- should be
    # impossible (merge drops them), a non-zero count here is a merge bug.
    ...
    ok = sum(st.values()) == expected_ledger_rows and not cross_company_dupes and not banned_leaked
    ...
    assert not banned_leaked, (
        ...
        "merge_bestsellers.py's BANNED filter should have caught these"
    )
