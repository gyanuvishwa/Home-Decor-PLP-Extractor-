#!/usr/bin/env python3
"""Build Bestsellers.xlsx from the bestsellers worker JSON (bs<SR>.json).
...
Writes a BRAND NEW workbook. Never opens, reads or touches:
  ...
  Final_Company_List (1).xlsx / Clearance.xlsx / New_Arrivals.xlsx
...
Sheets written:
  ...
  Review            - leaf rows with null qty, a MANUAL REVIEW flag, or a
                       PDP/editorial-page suspicion the worker or this merge
                       could not resolve with confidence

This is PLP-LEVEL extraction, not a product taxonomy. Every row's category is
"Bestsellers" (or a genuine site-defined child of it, e.g. "Furniture
Bestsellers"); sub_category is null unless the site genuinely exposes a
sub-listing. Trusts the worker's PLP-vs-marketing judgement
(rules/bestsellers.md) - this merge does not re-derive it, it only nets:
navigation/marketing nodes that are obviously not a Bestsellers PLP (BANNED,
below), individual PDP links masquerading as a PLP link (PDP_LIKE), and the
same structural hygiene (dedup key, qty+evidence, link validity, ASCII-only)
every category shares.

Idempotent: rebuilds everything from whatever bs*.json is on disk.

Usage:  python merge_bestsellers.py
"""
...
BOOK = os.path.join(PROJ, "Bestsellers.xlsx")
...
# Navigation/marketing nodes that are never a genuine Bestsellers PLP, even if
# discovered while looking for one (rules/bestsellers.md SS9). Deliberately does
# NOT ban "best sellers/bestsellers/top sellers/popular/trending/customer
# favorites" etc. - those ARE the target concept for this skill.
BANNED = re.compile(
    ...
    r'new\s?arrivals?|new\s?in|just\s?in|sale|offers?|clearance|outlet|final\s?sale|'
...
# A link that resolves to a single product page, not a listing. This is a
# structural safety net for the rules module's core distinction ("a product
# marked Bestseller is not a Bestseller PLP") - it only catches the common
# URL shapes; it does not replace the worker's own page inspection.
PDP_LIKE = re.compile(
    r'/(products?|item|p)/[^/]+/?$|[?&](variant|sku)=', re.I)
...
# company). Dropped entirely, not just cleared, per the user's standing
# instruction. Ledger total is intentionally 286 - len(this dict).
...
def load_blocks(scope):
    ...
    for c in scope:
        sr = c["sr"]
        p = os.path.join(SCRATCH, f"bs{sr}.json")
        if not os.path.exists(p):
            blocks.append({**c, "status": "MISSING", "failure_reason":
                           f"no bs{sr}.json produced", "notes": None, "rows": []})
            problems.append(f"SR {sr}: NO FILE (bs{sr}.json missing)")
            continue
        try:
        ...
        for r in d.get("rows") or []:
            ...
            cat = (r.get("category") or "").strip() or "Bestsellers"
            ...
            if not sub:
                # Bestsellers rows are commonly a single leaf named "Bestsellers"
                # itself - allow sub == category in that case rather than
                # requiring an invented sub-category name.
                if cat:
                    sub = cat
                else:
            ...
            if not is_parent and link and PDP_LIKE.search(link) and "PDP" not in (flag or ""):
            ...
            if is_parent:
                ...
                # different listing page - keep both.
...
def ledger_status(b):
    ...
    if st == "partial" and not b["rows"]:
        ...
        return "BLOCKED / ACCESS FAILURE"
    return "BESTSELLERS FOUND" if b["rows"] else "NO BESTSELLERS FOUND"


def main():
    for f in ("Bestsellers.xlsx",):
    ...
    LHDR = ["Maisons", "Company", "Country", "Website", "Bestsellers Found",
            "Bestsellers Record Count", "Status", "Notes"]
    lw.append(LHDR)
    for c in range(1, len(LHDR) + 1):
        ...
        cell.border = CELL_BORDER
        cell.alignment = Alignment(vertical="center", horizontal="center")
    counts = {"BESTSELLERS FOUND": 0, "NO BESTSELLERS FOUND": 0,
